...
describe('Hydra wordlists', () => {
  ...
  it('persists added wordlists in localStorage', async () => {
    ...
    const {
      unmount
    } = render(/*#__PURE__*/_jsx(HydraApp, {}));
    fireEvent.change(screen.getByTestId('user-file-input'), {
      target: {
        files: [file]
      }
    });
    await screen.findByText('users.txt', {
      selector: 'li'
    });
    unmount();
    render(/*#__PURE__*/_jsx(HydraApp, {}));
...
describe('Hydra pause and resume', () => {
  beforeEach(() => {
    localStorage.setItem('hydraUserLists', JSON.stringify([{
      name: 'u',
      content: 'a'
    }]));
    localStorage.setItem('hydraPassLists', JSON.stringify([{
      name: 'p',
      content: 'b'
    }]));
  });
  it('pauses and resumes cracking progress', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith('/api/hydra', expect.objectContaining({
      body: JSON.stringify({
        action: 'pause'
      })
    }));
