import { createGame, guess, useHint, isWinner, isLoser, isGameOver, } from '../apps/hangman/engine';
describe('hangman engine', () => {
    ...
    test('hint reveals one new letter', () => {
        ...
        expect(game.guessed.includes(first)).toBe(true);
        const second = useHint(game);
        expect(second && first !== second).toBe(true);
        expect(game.guessed.includes(second)).toBe(true);
    });
