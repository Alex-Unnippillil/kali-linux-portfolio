...
describe.skip('App component smoke tests', () => {
  ...
  const targets = entries.flatMap(entry => {
    ...
    if (stat.isDirectory()) {
      const candidate = ['index.tsx', 'index.ts', 'index.jsx', 'index.js', `${entry}.tsx`, `${entry}.ts`, `${entry}.jsx`, `${entry}.js`].find(file => fs.existsSync(path.join(fullPath, file)));
  ...
  targets.forEach(file => {
    const importPath = path.relative(__dirname, file).replace(/\\/g, '/');
