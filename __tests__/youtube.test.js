...
describe('YouTubeApp', () => {
  ...
  it('shows message when API key is missing', () => {
    ...
    render(/*#__PURE__*/_jsx(YouTubeApp, {}));
    expect(screen.getByText(/YouTube API key is not configured/i)).toBeInTheDocument();
  });
  it('renders video cards with thumbnail and metadata', () => {
    ...
    mockVideos.forEach(video => {
      ...
      expect(screen.getByText(`${video.playlist} • ${new Date(video.publishedAt).toLocaleDateString()}`)).toBeInTheDocument();
    });
  });
  ...
  it('allows sorting after switching categories', async () => {
    ...
    const getTitles = () => screen.getAllByRole('link').map(a => a.textContent);
  ...
  it('sort options reorder videos', async () => {
    ...
    const getTitles = () => screen.getAllByRole('link').map(a => a.textContent);
    expect(getTitles()).toEqual(['Cooking with React', 'React Tutorial', 'Advanced React']);
    await user.selectOptions(screen.getByLabelText(/sort by/i), 'title');
    expect(getTitles()).toEqual(['Advanced React', 'Cooking with React', 'React Tutorial']);
  });
  it('handles videos missing metadata when sorting', async () => {
    const user = userEvent.setup();
    const videosWithMissing = [...mockVideos, {
      id: '4',
      thumbnail: 'thumb4.jpg',
      url: 'https://youtu.be/4'
    }];
    render(/*#__PURE__*/_jsx(YouTubeApp, {
      initialVideos: videosWithMissing
    }));
  ...
  it('renders videos with missing properties without crashing', () => {
    const incomplete = [{
      id: '5',
      url: 'https://youtu.be/5'
    }];
    render(/*#__PURE__*/_jsx(YouTubeApp, {
      initialVideos: incomplete
    }));
    expect(screen.getAllByTestId('video-card')).toHaveLength(1);
  });
  it('category buttons include transition-colors class', () => {
    ...
    screen.getAllByRole('tab').forEach(btn => expect(btn).toHaveClass('transition-colors'));
  });
  it('video cards include transition class', () => {
    render(/*#__PURE__*/_jsx(YouTubeApp, {
      initialVideos: mockVideos
    }));
    screen.getAllByTestId('video-card').forEach(card => expect(card).toHaveClass('transition'));
  });
  it('memoizes categories and sorted lists between renders', async () => {
    ...
    const {
      default: MemoApp
    } = await import('../components/apps/youtube');
    const {
      rerender,
      unmount
    } = ReactTesting.render(/*#__PURE__*/_jsx(MemoApp, {
      initialVideos: mockVideos
    }));
    ...
    rerender(/*#__PURE__*/_jsx(MemoApp, {
      initialVideos: mockVideos
    }));
    expect(calls[0]).toHaveBeenCalledTimes(1);
    expect(calls[2]).toHaveBeenCalledTimes(1);
  ...
  it('fetches videos from multiple playlists via Promise.all', async () => {
    ...
    global.fetch = jest.fn(url => {
      if (url.includes('channels')) return Promise.resolve({
        json: () => responses.channel
      });
      if (url.includes('playlists?')) return Promise.resolve({
        json: () => responses.playlists
      });
    ...
    expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('playlistItems?part=snippet&playlistId=pl1'));
    expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('playlistItems?part=snippet&playlistId=pl2'));
  });
});
