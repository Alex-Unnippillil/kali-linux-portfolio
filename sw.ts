...
setCacheNameDetails({ prefix: buildId });
const withPrefix = (name: string) => `${buildId}-${name}`;
...
registerRoute(
  ...
  new NetworkFirst({ cacheName: withPrefix('app-index') }),
);
...
registerRoute(
  ({ url }) => url.pathname.startsWith('/wallpapers/'),
    new CacheFirst({
      cacheName: withPrefix('wallpapers'),
      plugins: [
        new ExpirationPlugin({
          maxEntries: 60,
...
registerRoute(
  ({ url }) => url.pathname.startsWith('/apps/'),
    new CacheFirst({
      cacheName: withPrefix('game-assets'),
      plugins: [
        new ExpirationPlugin({
          maxEntries: 60,
          maxAgeSeconds: 30 * 24 * 60 * 60,
...
registerRoute(
  ({ url }) => url.pathname.endsWith('.json') && url.pathname !== '/projects.json',
    new StaleWhileRevalidate({
      cacheName: withPrefix('json-feeds'),
      plugins: [
        new ExpirationPlugin({
          maxEntries: 30,
