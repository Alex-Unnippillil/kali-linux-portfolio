...
import { setI18nLocale } from './useI18n';
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  const [locale, setLocaleState] = useState<string>(defaults.locale);
  const [localeHydrated, setLocaleHydrated] = useState(false);
  const localeUserOverride = useRef(false);
  ...
  useEffect(() => {
    ...
    saveLocale(locale);
    setI18nLocale(locale, { persist: false });
    ...
    <SettingsContext.Provider
      value={{
        ...
        setLocale: (value: string) => {
          localeUserOverride.current = true;
          setLocaleHydrated(true);
