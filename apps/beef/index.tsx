...
type TimelineStep = { label: string; state: TimelineState };
...
const BeefPage: React.FC = () => {
  ...
  if (!managerRef.current) {
    managerRef.current = new WindowManager({
      bounds: { left: 0, top: 0, width: 960, height: 512 },
      visibility: 'normal',
    });
  }
  ...
  const windowState = snapshot.visibility;
  ...
  const handleMaximize = () => {
    if (manager.getSnapshot().visibility === 'maximized') {
      manager.restore();
      return;
    }
    ...
    manager.maximize({
      ...
      taskbarInset: measureSnapBottomInset(),
      safeArea: {
  ...
  const frameClasses = useMemo(
    () =>
      clsx(
        'relative mx-auto flex min-h-[32rem] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-kali-border/70 bg-[color:var(--kali-panel)] shadow-[0_24px_64px_rgba(15,148,210,0.16)] backdrop-blur transition-all duration-300 ease-in-out',
        isMaximized && 'max-w-none border-kali-accent/70 shadow-[0_0_60px_rgba(15,148,210,0.32)]',
        isMinimized && 'pointer-events-none scale-[0.98] opacity-70',
      ),
      ...
      <div className="flex h-full w-full flex-col bg-kali-background text-kali-text">
        <header className="flex flex-col gap-4 border-b border-kali-border/60 bg-[color:var(--kali-overlay)] p-4 backdrop-blur-sm lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex items-center gap-2 self-end text-kali-text/80 lg:self-start">
            <Image src="/themes/Yaru/window/window-minimize-symbolic.svg" alt="minimize" width={24} height={24} />
            <Image src="/themes/Yaru/window/window-close-symbolic.svg" alt="close" width={24} height={24} />
          </div>
        </header>
        <main className="flex flex-1 flex-col gap-6 p-6">
          <section className="space-y-3 text-center">
            ...
            <p className="text-sm text-kali-text/70">
              The BeEF demo window is closed. Reopen it to continue exploring the guided simulation.
            </p>
            <button
              type="button"
              onClick={restoreWindow}
              className="rounded-full bg-kali-primary px-4 py-2 text-sm font-semibold text-kali-inverse shadow-lg transition-colors hover:bg-[color:color-mix(in_srgb,var(--color-primary)_82%,var(--kali-overlay))]"
            >
              Reopen window
            </button>
          </section>
          <section className="px-2" aria-label="BeEF campaign timeline">
  ...
  return (
    <div className="flex min-h-screen w-full items-center justify-center bg-kali-background/95 p-4 text-kali-text">
      <WindowShell
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="sr-only" aria-live="polite">
                  {isMinimized ? 'BeEF Demo window minimized' : 'BeEF Demo window active'}
                </span>
                {isMaximized && (
        ...
        bodyClassName="flex-1 overflow-auto bg-[color:color-mix(in_srgb,var(--kali-panel)_65%,transparent)] p-4 pt-2"
        footer={
      ...
      {isMinimized && (
        <div className="mt-6 flex flex-col items-center gap-3 text-center text-sm text-kali-text/80" aria-live="polite">
          <p>BeEF Demo is minimized. Restore window to continue.</p>
          <button
            type="button"
            onClick={restoreFromMinimize}
            className="rounded-full border border-kali-border/60 px-4 py-1.5 font-semibold text-kali-text transition-colors hover:bg-[color:var(--kali-overlay)]"
          >
            Restore window
          </button>
        </div>
