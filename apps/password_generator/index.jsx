...
const PasswordGenerator = () => {
    ...
    return (<div className="h-full w-full bg-gray-900 text-white p-4 flex flex-col space-y-4">
      <div>
        <label htmlFor="length" className="mr-2">Length:</label>
        <input id="length" type="number" min={4} max={64} value={length} onChange={(e) => setLength(parseInt(e.target.value, 10) || 0)} className="text-black px-2"/>
      </div>
      ...
      <div className="flex space-x-2 items-center">
        <input data-testid="password-display" type="text" readOnly value={password} className="flex-1 text-black px-2 py-1 font-mono leading-[1.2] focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500"/>
        <button type="button" onClick={copyToClipboard} className="px-3 py-1 bg-blue-600 rounded focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500">
          Copy
        </button>
      </div>
      <div>
      ...
      <div className="mt-auto">
        <button type="button" onClick={generatePassword} className="w-full px-4 py-2 bg-green-600 rounded">
          Generate
        </button>
      </div>
    </div>);
