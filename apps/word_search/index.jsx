...
const WordSearchInner = ({ getDailySeed }) => {
    const router = useRouter();
    const { seed: seedQuery, words: wordsQuery } = router.query;
    ...
    useEffect(() => {
        try {
            ...
            if (saved) {
                ...
                setFound(new Set(data.found));
                setFoundCells(new Set(data.foundCells));
                setHintCells(new Set(data.hintCells || []));
    ...
    useEffect(() => {
        ...
        const init = async () => {
            const queryWords = typeof wordsQuery === 'string'
    ...
    const newPuzzle = () => {
        ...
        router.replace({ pathname: router.pathname, query: { seed: newSeed, words: words.join(',') } }, undefined, { shallow: true });
    };
    const restart = () => {
    ...
    const useFirstHint = () => {
        if (firstHints <= 0)
            return;
        const remaining = placements.filter((p) => !found.has(p.word) && !hintCells.has(key(p.positions[0])));
        if (!remaining.length)
            return;
    ...
    return (<div className="p-4 select-none">
      <div className="flex flex-wrap gap-2 mb-2 print:hidden items-center">
        <svg viewBox="0 0 36 36" className="w-10 h-10">
          <circle className="text-gray-300" stroke="currentColor" strokeWidth="4" fill="transparent" r={radius} cx="18" cy="18"/>
          <circle className="text-green-500" stroke="currentColor" strokeWidth="4" fill="transparent" strokeDasharray={circumference} strokeDashoffset={circumference - progress * circumference} strokeLinecap="round" r={radius} cx="18" cy="18" style={{ transition: 'stroke-dashoffset 0.5s' }}/>
        ...
        <button type="button" onClick={() => inputRef.current?.click()} className="px-2 py-1 bg-purple-700 text-white rounded">
          Import LB
        </button>
        <input ref={inputRef} type="file" className="hidden" onChange={importLeaderboard}/>
        <button type="button" onClick={useFirstHint} disabled={firstHints <= 0} className="px-2 py-1 bg-teal-700 text-white rounded disabled:opacity-50">
          First Hint ({firstHints})
        </button>
        <button type="button" onClick={useLastHint} disabled={lastHints <= 0} className="px-2 py-1 bg-teal-700 text-white rounded disabled:opacity-50">
          Last Hint ({lastHints})
        </button>
        <label className="flex items-center space-x-1">
          <span className="text-sm">Quality</span>
          <input type="range" min="0.5" max="1" step="0.1" value={quality} onChange={(e) => setQuality(parseFloat(e.target.value))}/>
        </label>
        <label className="flex items-center space-x-1">
      ...
      <div style={{
        ...
        }} className={`grid border w-max ${highContrast ? 'contrast-200' : ''}`} onMouseLeave={handleMouseUp}>
        {grid.map((row, r) => row.map((letter, c) => {
            ...
            return (<div key={posKey} onMouseDown={() => handleMouseDown(r, c)} onMouseEnter={() => handleMouseEnter(r, c)} onMouseUp={handleMouseUp} className={`w-8 h-8 flex items-center justify-center border text-sm font-bold cursor-pointer select-none ${isFound ? 'bg-green-300' : isSelected ? 'bg-yellow-300' : isHint ? 'bg-blue-200' : 'bg-white'}`} aria-label={`row ${r + 1} column ${c + 1} letter ${letter}`}>
                {letter}
              </div>);
        }))}
      </div>
      <ul className="mt-4 columns-2 md:columns-3">
        {words.map((w) => (<li key={w} className={`relative overflow-hidden w-fit ${found.has(w) ? 'line-through text-gray-500' : ''}`}>
            <span className={found.has(w) ? 'word-found' : ''}>{w}</span>
          </li>))}
      </ul>
    </div>);
};
const WordSearch = ({ getDailySeed }) => (<SettingsProvider>
