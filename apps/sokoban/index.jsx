...
import { LEVEL_PACKS, parseLevels } from './levels';
import { loadLevel, move, undo as undoMove, reset as resetLevel, reachable, isSolved, directionKeys, findHint, wouldDeadlock, } from './engine';
const CELL = 32;
const Sokoban = () => {
    const [packs, setPacks] = useState(LEVEL_PACKS);
    const [packIndex, setPackIndex] = useState(0);
    const [index, setIndex] = useState(0);
    const currentPack = packs[packIndex];
    const [state, setState] = useState(() => loadLevel(currentPack.levels[0]));
    const [reach, setReach] = useState(reachable(loadLevel(currentPack.levels[0])));
    const [best, setBest] = useState(null);
    const [hint, setHint] = useState('');
    const [status, setStatus] = useState('');
    const [warnDir, setWarnDir] = useState(null);
    const [ghost, setGhost] = useState(new Set());
    const [puffs, setPuffs] = useState([]);
    const puffId = React.useRef(0);
    const selectLevel = useCallback((i, pIdx = packIndex, pData = packs) => {
    ...
    const selectPack = useCallback((pIdx) => {
        selectLevel(0, pIdx);
    }, [selectLevel]);
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (code) {
                ...
                if (parsed.length) {
                    const customPack = { name: 'Custom', difficulty: 'Custom', levels: parsed };
                    const newPacks = [...LEVEL_PACKS, customPack];
                    setPacks(newPacks);
    ...
    const cellStyle = useMemo(() => ({ width: CELL, height: CELL }), []);
    const keyPos = useCallback((p) => `${p.x},${p.y}`, []);
    ...
    return (<div className="p-4 space-y-2 select-none">
      <div className="flex flex-wrap space-x-2 mb-2 items-center">
        <select value={packIndex} onChange={(e) => selectPack(Number(e.target.value))}>
          {packs.map((p, i) => (<option key={p.name} value={i}>{`${p.name} (${p.difficulty})`}</option>))}
        </select>
        <select value={index} onChange={(e) => selectLevel(Number(e.target.value))}>
          {currentPack.levels.map((_, i) => (<option key={i} value={i}>{`Level ${i + 1}`}</option>))}
        </select>
      ...
      <div className="relative bg-gray-700" style={{ width: state.width * CELL, height: state.height * CELL }} onMouseLeave={() => setGhost(new Set())}>
        {Array.from({ length: state.height }).map((_, y) => Array.from({ length: state.width }).map((_, x) => {
            const k = `${x},${y}`;
            ...
            return (<React.Fragment key={k}>
                <div className={`absolute ${isWall ? 'bg-gray-800' : 'bg-gray-600'} ${isTarget ? 'ring-2 ring-yellow-400' : ''}`} style={{ ...cellStyle, left: x * CELL, top: y * CELL }} onMouseEnter={() => handleHover(x, y)}/>
                {isReach && !isWall && (<div className="absolute bg-green-400 opacity-30" style={{ ...cellStyle, left: x * CELL, top: y * CELL }}/>)}
                {inGhost && (<div className="absolute bg-blue-300 opacity-40" style={{ ...cellStyle, left: x * CELL, top: y * CELL }}/>)}
              </React.Fragment>);
        }))}
        {Array.from(state.boxes).map((b) => {
            const [x, y] = b.split(',').map(Number);
            const dead = state.deadlocks.has(b);
            return (<div key={b} className={`absolute transition-transform duration-100 ${dead ? 'bg-red-500' : 'bg-orange-400'}`} style={{
                    ...cellStyle,
                    transform: `translate(${x * CELL}px, ${y * CELL}px)`,
                }}/>);
        })}
        {puffs.map((p) => (<div key={p.id} className="absolute pointer-events-none w-4 h-4 bg-gray-200 opacity-70 rounded-full animate-ping" style={{ left: p.x * CELL + CELL / 2 - 8, top: p.y * CELL + CELL / 2 - 8 }}/>))}
        <div className="absolute bg-blue-400 transition-transform duration-100" style={{
            ...cellStyle,
