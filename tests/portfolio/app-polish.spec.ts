...
for (const viewport of [{ width: 390, height: 844 }, { width: 1440, height: 900 }]) {
  test(`working apps retain input and notes at ${viewport.width}px`, async ({ browser, baseURL }) => {
    const phone = viewport.width < 640;
    const context = await browser.newContext({ viewport, hasTouch: phone, isMobile: phone, baseURL });
    ...
    try {
      ...
      await display.fill('6*7'); await display.press('Enter');
      ...
      await page.screenshot({ path: `portfolio-screenshots/polished-apps-${viewport.width}.png`, animations: 'disabled' });
