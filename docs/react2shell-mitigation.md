...
## Context
- Stack: Next.js 16.0.10 with React/ReactDOM 19.2.3 (patched baseline).
...
## Current status
- Framework packages upgraded in lockstep (Next.js/React/ReactDOM/@next/bundle-analyzer/eslint-config-next) and lockfile regenerated.
- Shared sanitizer (`lib/sanitize.ts`) enforces escaping plus anchor hardening across hydration surfaces.

## Upgrade plan
1. Keep **next**, **react**, **react-dom**, **@next/bundle-analyzer**, and **eslint-config-next** in lockstep when new patched releases are published so the Yarn lockfile stays consistent.
...
| --- | --- | --- |
| `components/tweet-embed.js` | Twitter syndication HTML | Sanitized via shared `sanitizeHtml`; auto-adds rel/target restrictions on links. |
| `components/HelpPanel.tsx` | Markdown fetched from `/docs/apps/*.md` | Parsed with `marked` and sanitized via shared helper before injection. |
...
| `components/apps/ascii_art/index.js` | Locally generated ASCII HTML view | Derived from user typing or canvas sampling; sanitized before display. |
| `components/apps/ettercap/index.js` | Filter highlighting | Highlight helper escapes and wraps matches via shared utilities. |
