...
const boardsEqual = (a, b) => a.every((row, r) => row.every((cell, c) => cell === b[r][c]));
...
const checkHighest = (board) => {
    let m = 0;
    board.forEach((row) => row.forEach((v) => { if (v > m)
        m = v; }));
    return m;
};
...
const Page2048 = () => {
    ...
    const handleMove = useCallback((dir) => {
        ...
        if (!moved || boardsEqual(board, moved))
            return;
    ...
    return (<div className="h-full w-full bg-gray-900 text-white p-4 flex flex-col space-y-4">
      <div className="flex space-x-2">
        ...
        <select className="text-black px-1 rounded" value={boardType} onChange={(e) => setBoardType(e.target.value)}>
      ...
      <div className="grid grid-cols-4 gap-2">
        {board.map((row, rIdx) => row.map((cell, cIdx) => (<div key={`${rIdx}-${cIdx}`} className={`h-16 w-16 flex items-center justify-center text-2xl font-bold rounded ${cell ? tileColors[cell] || 'bg-gray-700' : 'bg-gray-800'}`}>
              {displayCell(cell)}
            </div>)))}
      </div>
      {(won || lost) && (<div className="mt-4 text-xl">{won ? 'You win!' : 'Game over'}</div>)}
    </div>);
};
