...
const NmapNSEPage = () => {
    ...
    const openScriptDoc = useCallback((name) => {
        if (typeof window !== 'undefined') {
            window.open(`https://nmap.org/nsedoc/scripts/${name}.html`, '_blank', 'noopener,noreferrer');
        }
    }, []);
    useEffect(() => {
        const load = async () => {
    ...
    return (<div className="p-4 bg-gray-900 text-white min-h-screen">
      ...
      {Object.entries(data).map(([category, scripts]) => (<div key={category} className="mb-6">
          <h2 className="text-xl mb-2 capitalize">{category}</h2>
          {scripts.map((script) => (<div key={script.name} className="mb-4">
              <button type="button" onClick={() => openScriptDoc(script.name)} className="font-mono text-blue-400 underline">
                {script.name}
              </button>
              ...
              <button type="button" onClick={() => copyExample(script.example)} className="mt-2 px-2 py-1 bg-blue-700 rounded focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500">
                Copy
              </button>
            </div>))}
        </div>))}
