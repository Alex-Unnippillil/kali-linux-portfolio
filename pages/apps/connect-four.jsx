...
const evaluateWindow = (window, player) => {
    ...
    if (playerCount === 4)
        score += 100;
    else if (playerCount === 3 && empty === 1)
        score += 5;
    else if (playerCount === 2 && empty === 2)
        score += 2;
...
const minimax = (board, depth, alpha, beta, maximizing) => {
    ...
    const isTerminal = checkWinner(board, 'red') ||
...
const ConnectFour = () => {
    ...
    const finalizeMove = React.useCallback((newBoard, color, col) => {
    ...
    const dropDisc = React.useCallback((col, color = player) => {
        if (winner || animDisc)
            return;
        if (color !== player)
            return;
    ...
    const cellHighlight = (rIdx, cIdx) => {
        if (winningCells.some((p) => p.r === rIdx && p.c === cIdx))
            return 'ring-4 ring-white';
        if (teaching.wins.some((line) => line.some((p) => p.r === rIdx && p.c === cIdx)))
            return 'ring-4 ring-green-400';
        if (teaching.threats.some((line) => line.some((p) => p.r === rIdx && p.c === cIdx)))
            return 'ring-4 ring-red-400';
        return '';
    };
    return (<div className="h-full w-full flex flex-col items-center justify-center bg-ub-cool-grey text-white p-4">
      {winner && (<div className="mb-2 capitalize">
          {winner === 'draw' ? 'Draw!' : `${winner} wins!`}
        </div>)}
      <div className="relative" onMouseLeave={() => setSelectedCol(null)}>
        <div className="grid grid-cols-7 gap-1">
          {board.map((row, rIdx) => row.map((cell, cIdx) => (<div key={`${rIdx}-${cIdx}`} className={`h-10 w-10 flex items-center justify-center cursor-pointer bg-blue-700 ${cellHighlight(rIdx, cIdx)}`} onClick={() => dropDisc(cIdx, 'yellow')} onMouseEnter={() => setSelectedCol(cIdx)}>
                {cell && (<div className={`h-8 w-8 rounded-full ${cell === 'red' ? 'bg-red-500' : 'bg-yellow-400'}`}/>)}
              </div>)))}
        </div>
        {selectedCol !== null && (<div className="absolute top-0 pointer-events-none bg-gradient-to-b from-black/30 to-transparent" style={{
                left: selectedCol * SLOT,
                width: CELL_SIZE,
                height: BOARD_HEIGHT,
            }}/>)}
        {winColumn !== null && (<div className="absolute" style={{ left: winColumn * SLOT + CELL_SIZE / 2 - 4, top: -8 }}>
            <div className="h-2 w-2 rounded-full bg-green-400"/>
          </div>)}
        {animDisc && (<div className="absolute left-0 top-0" style={{
                transform: `translateX(${animDisc.col * SLOT}px) translateY(${animDisc.y}px)`,
            }}>
            <div className={`h-8 w-8 rounded-full ${animDisc.color === 'red' ? 'bg-red-500' : 'bg-yellow-400'}`}/>
          </div>)}
      </div>
      <div className="mt-4 flex flex-col items-center gap-2">
        <div className="flex items-center gap-2">
          ...
          <input id="ai-depth" type="range" min={1} max={6} value={aiDepth} onChange={(e) => setAiDepth(parseInt(e.target.value, 10))} className="w-32"/>
        </div>
        <button className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded" onClick={rematch}>
          Rematch
        </button>
      </div>
    </div>);
