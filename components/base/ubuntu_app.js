...
export class UbuntuApp extends Component {
  ...
  handlePrefetch = () => {
    if (!this.state.prefetched && typeof this.props.prefetch === "function") {
      ...
      prefetchDynamicImport(
        this.props.prefetch,
        `/apps/${this.props.id}.js`,
      ).finally(() => {
        requestAnimationFrame(() =>
          this.setState({ prefetched: true, prefetching: false }),
        );
      });
    }
  };
  ...
  render() {
    const prefetchHint = this.state.prefetching ? " ring-1 ring-white" : "";
    return (
      <button
        type="button"
        ...
        className={
          ...
          prefetchHint +
          " p-1 m-0 z-10 bg-white bg-opacity-0 hover:bg-opacity-20 focus:bg-white focus:bg-opacity-50 focus:border-yellow-700 focus:border-opacity-100 border border-transparent rounded select-none w-20 h-18 flex flex-col justify-start items-center text-center text-xs font-normal text-white transition-hover transition-active "
        }
