...
export class Window extends Component {
  ...
  handleTouchStart = (e) => {
    if (e.touches.length === 1) {
      const target = e.target.closest(".bg-ub-window-title");
      this._touchFromTitleBar = !!target;
      if (this._touchFromTitleBar) {
  ...
  handleTouchMove = (e) => {
    if (
      ...
      if (dist < this._pinchStartDist - 30) {
        e.preventDefault();
        this.restoreWindow();
        this._pinchStartDist = null;
      }
  ...
  handleTouchEnd = (e) => {
    if (this._touchFromTitleBar && e.changedTouches.length === 1) {
      const touch = e.changedTouches[0];
      const dy = touch.clientY - this._touchStartY;
      const dx = touch.clientX - this._touchStartX;
      const dt = Date.now() - this._touchStartTime;
      if (dy > 120 && Math.abs(dx) < 80 && dt < 300) {
        this.minimizeWindow();
      }
    }
    this._touchFromTitleBar = false;
