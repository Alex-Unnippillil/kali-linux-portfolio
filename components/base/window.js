...
export class Window extends Component {
  ...
  toggleShade = () => {
    this.setState((prev) => {
      if (prev.shaded) {
        const height =
          prev.lastSize && prev.lastSize.height
            ? prev.lastSize.height
            : prev.height;
        return { shaded: false, height };
      } else {
        return {
          shaded: true,
          lastSize: { ...prev.lastSize, height: prev.height },
          height: 8,
        };
      }
    }, this.resizeBoundries);
  ...
  snapWindow = (position) => {
    ...
    if (r) {
      ...
      r.style.setProperty(
        "--window-transform-x",
        x.toFixed(1).toString() + "px",
      );
      r.style.setProperty(
        "--window-transform-y",
        y.toFixed(1).toString() + "px",
      );
      prevBounds = { x, y };
    }
  ...
  checkSnapPreview = () => {
    ...
    if (nearLeft && nearTop) {
      ...
      this.setSnapPreview(snap, "top-left");
    } else if (nearRight && nearTop) {
      snap = { left: "50%", top: "0", width: "50%", height: "50%" };
      this.setSnapPreview(snap, "top-right");
    } else if (nearLeft && nearBottom) {
      snap = { left: "0", top: "50%", width: "50%", height: "50%" };
      this.setSnapPreview(snap, "bottom-left");
    } else if (nearRight && nearBottom) {
      snap = { left: "50%", top: "50%", width: "50%", height: "50%" };
      this.setSnapPreview(snap, "bottom-right");
    } else if (nearLeft) {
      snap = { left: "0", top: "0", width: "50%", height: "100%" };
      this.setSnapPreview(snap, "left");
    } else if (nearRight) {
      snap = { left: "50%", top: "0", width: "50%", height: "100%" };
      this.setSnapPreview(snap, "right");
    } else if (nearTop) {
      snap = { left: "0", top: "0", width: "100%", height: "50%" };
      this.setSnapPreview(snap, "top");
    } else if (nearBottom) {
      snap = { left: "0", top: "50%", width: "100%", height: "50%" };
      this.setSnapPreview(snap, "bottom");
    } else {
      if (this.state.snapPreview) this.clearSnapPreview();
    }
  };
  ...
  handleKeyDown = (e) => {
    ...
    if (this.state.resizing) {
      ...
      if (e.key === "Enter" || e.key === "Escape") {
        ...
        e.stopPropagation();
        this.setState(
          (prev) => ({ width: Math.max(prev.width - step, 20) }),
          this.resizeBoundries,
        );
        ...
        e.stopPropagation();
        this.setState(
          (prev) => ({ width: Math.min(prev.width + step, 100) }),
          this.resizeBoundries,
        );
        ...
        e.stopPropagation();
        this.setState(
          (prev) => ({ height: Math.max(prev.height - step, 20) }),
          this.resizeBoundries,
        );
        ...
        e.stopPropagation();
        this.setState(
          (prev) => ({ height: Math.min(prev.height + step, 100) }),
          this.resizeBoundries,
        );
      }
      this.focusWindow();
        ...
        >
          <div
            ...
            style={{
              ...
              zIndex:
                (this.props.zIndex || 0) + (this.state.alwaysOnTop ? 1000 : 0),
            }}
            className={
...
export function WindowTopBar({ title, onKeyDown, onBlur, grabbed }) {
  return (
    <button
      type="button"
      className={
        " relative bg-ub-window-title border-t-2 border-white border-opacity-5 px-3 text-white w-full select-none rounded-b-none flex items-center h-11"
      }
      tabIndex={0}
