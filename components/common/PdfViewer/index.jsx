...
const PdfViewer = ({ url }) => {
    ...
    return (<div>
      ...
      <div className="flex gap-2 overflow-x-auto mt-2">
        {thumbs.map((t, i) => (<canvas key={i} data-testid={`thumb-${i + 1}`} onClick={() => setPage(i + 1)} ref={(el) => {
                if (el)
                    el.getContext('2d')?.drawImage(t, 0, 0);
            }} width={t.width} height={t.height}/>))}
      </div>
      {matches.length > 0 && (<div data-testid="search-results">
          {matches.map((m) => (<div key={m}>Page {m}</div>))}
        </div>)}
    </div>);
};
