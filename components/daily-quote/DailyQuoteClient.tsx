...
const DailyQuoteClient: React.FC = () => {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-ub-cool-grey p-4 text-white">
      <div
        ref={cardRef}
        className="group relative rounded bg-gradient-to-br from-[var(--color-primary)]/30 to-[var(--color-secondary)]/30 p-6 text-center text-white shadow-lg"
      >
        {quote ? (
          <div key={quote.content} className="animate-quote">
            <span className="absolute -top-4 left-4 select-none text-[64px] text-white/20" aria-hidden="true">
              &ldquo;
            </span>
            <p className="mb-4 text-[18px] tracking-[6px] leading-[24px] sm:text-[20px] sm:leading-[26px]">
              {quote.content}
            </p>
            ...
            <div className="absolute top-2 right-2 flex gap-2 opacity-0 transition group-hover:opacity-100">
              <button
                onClick={copyQuote}
                className="rounded bg-black/30 p-1 hover:bg-black/50"
          ...
          <p className="text-sm text-white/70">Loading quote…</p>
        )}
      </div>
...
export default DailyQuoteClient;
