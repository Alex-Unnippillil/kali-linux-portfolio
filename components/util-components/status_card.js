...
export default function StatusCard(props) {
  ...
  useEffect(() => {
    const storedSound = Number(localStorage.getItem('sound-level') || 75);
    const storedBrightness = Number(localStorage.getItem('brightness-level') || 100);
    setSoundLevel(storedSound);
    setBrightnessLevel(storedBrightness);
    document.getElementById('monitor-screen').style.filter = `brightness(${(3 / 400) * storedBrightness + 0.25})`;
  }, []);

  const handleBrightness = (e) => {
    const value = Number(e.target.value);
    setBrightnessLevel(value);
    localStorage.setItem('brightness-level', String(value));
  ...
  const handleSound = (e) => {
    const value = Number(e.target.value);
    setSoundLevel(value);
    localStorage.setItem('sound-level', String(value));
  };
