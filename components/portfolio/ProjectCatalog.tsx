...
type SelectProject = (project: Project, button: HTMLButtonElement) => void;
export function ProjectCard({ project, compact = false, onSelect }: { project: Project; compact?: boolean; onSelect?: SelectProject }) {
  return (
    <article className={styles.projectCard}>
      ...
      <h3>{onSelect ? <button type="button" aria-label={project.title} className={styles.projectTitleButton} onClick={(event) => onSelect(project, event.currentTarget)}>{project.title}</button> : <Link href={`/projects/${project.slug}`} prefetch={false}>{project.title}</Link>}</h3>
...
export default function ProjectCatalog({ onSelect }: { onSelect?: SelectProject } = {}) {
  ...
  return (
    <section className={styles.catalog} aria-label="Project catalog">
      <div className={styles.catalogControls}>
        <div className={styles.search}><label htmlFor={inputId}>Find a project</label><div><Icon name="search" /><input aria-label="Find a project" id={inputId} type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search projects, skills, or technology" /></div></div>
        <div className={styles.filters} role="group" aria-label="Filter projects">{filters.map((name) => <button type="button" key={name} aria-label={name} aria-pressed={filter === name} onClick={() => setFilter(name)}>{name}</button>)}</div>
      </div>
      ...
      <div className={styles.projectGrid}>{filtered.map((project) => <ProjectCard key={project.slug} project={project} onSelect={onSelect} />)}</div>
