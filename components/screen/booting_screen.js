    ...
    return (
        <div
            style={{
                ...
                backgroundColor: 'var(--kali-overlay-solid)',
                color: 'var(--kali-text-strong)',
            }}
            className={(props.visible || props.isShutDown ? " visible opacity-100" : " invisible opacity-0 ") + " absolute duration-500 select-none flex flex-col justify-around items-center top-0 right-0 overflow-hidden m-0 p-0 h-screen w-screen"}>
            <Image
                width={400}
                height={400}
            ...
            <div className="w-10 h-10 flex justify-center items-center rounded-full outline-none cursor-pointer" onClick={props.turnOn} >
                {(props.isShutDown
                    ? <div
                        className="rounded-full flex justify-center items-center w-10 h-10 transition-colors hover:bg-[color:var(--kali-control-hover)]"
                        style={{
                            backgroundColor: 'var(--kali-control-surface)',
                            color: 'var(--color-inverse)',
                        }}
                    >
                        <Image width={32} height={32} className="w-8" src="/themes/Yaru/status/power-button.svg" alt="Power Button" sizes="32px" priority />
                    </div>
