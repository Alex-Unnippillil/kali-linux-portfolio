...
import { UbuntuCofIcon, PowerButtonIcon, ProcessWorkingIcon, UbuntuLogoIcon } from '../shell/hero-icons'
...
function BootingScreen(props) {
    ...
    return (
        <div
            ...
            <UbuntuCofIcon className="md:w-1/4 w-1/2" aria-hidden="true" />
            <div className="w-10 h-10 flex justify-center items-center rounded-full outline-none cursor-pointer" onClick={props.turnOn} >
                {(props.isShutDown
                    ? <div className="bg-white rounded-full flex justify-center items-center w-10 h-10 hover:bg-gray-300"><PowerButtonIcon className="w-8" aria-hidden="true" /></div>
                    : <ProcessWorkingIcon className={`w-10${props.visible ? ' animate-spin' : ''}`} aria-hidden="true" />)}
            </div>
            <UbuntuLogoIcon className="md:w-1/5 w-1/2" aria-hidden="true" />
