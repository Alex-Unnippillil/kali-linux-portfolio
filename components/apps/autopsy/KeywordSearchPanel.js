        ...
        {artifacts.map((a, idx) => (
          ...
          >
            <div className="flex items-center font-bold">
              ...
              <span
                dangerouslySetInnerHTML={{ __html: createTrustedHTML(highlight(a.name)) }}
              />
            </div>
            ...
            {a.user && (
              <div
                className="text-xs"
                dangerouslySetInnerHTML={{ __html: createTrustedHTML(`User: ${highlight(a.user)}`) }}
              />
            )}
            <div
              className="text-xs"
              dangerouslySetInnerHTML={{ __html: createTrustedHTML(highlight(a.description)) }}
            />
          </button>
        ))}
      </div>
