...
const supportsGamepadVibration = () => {
    ...
    return Array.from(pads).some((p) => p && p.vibrationActuator && typeof p.vibrationActuator.playEffect === 'function');
};
const triggerGamepad = (duration) => {
    if (!supportsGamepadVibration())
        return;
    try {
