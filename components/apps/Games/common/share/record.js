export async function recordCanvas(canvas, duration = 1000, options = { mimeType: 'video/webm' }) {
    // Fallback to PNG snapshot when MediaRecorder is unavailable
    if (typeof window === 'undefined' || typeof MediaRecorder === 'undefined') {
        return new Promise((resolve, reject) => canvas.toBlob((blob) => (blob ? resolve(blob) : reject(new Error('Snapshot failed'))), 'image/png'));
    }
