...
export default function YouTubeApp({ initialVideos = [] }) {
    ...
    useEffect(() => {
        if (!apiKey || initialVideos.length)
            return;
        (async () => {
            try {
                const res = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${CHANNEL_ID}&maxResults=25&order=date&type=video&key=${apiKey}`);
    ...
    return (<div className="h-full w-full overflow-auto bg-ub-cool-grey text-white p-4">
      <div className="mb-4">
        <input type="text" placeholder="Search" value={search} onChange={(e) => setSearch(e.target.value)} className="text-black px-3 py-2 rounded w-full max-w-sm"/>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {filtered.map((video) => (<div key={video.id} className="relative pb-[56.25%]">
            <iframe src={`${embedBase}/embed/${video.id}?enablejsapi=1&origin=${encodeURIComponent(origin)}`} title={video.title} className="absolute inset-0 w-full h-full" sandbox="allow-same-origin allow-scripts allow-popups" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" referrerPolicy="no-referrer" allowFullScreen loading="lazy"/>
            <div className="absolute inset-x-0 bottom-0 flex justify-between p-1 bg-black/60 text-xs">
              <button type="button" onClick={() => playVideo(video)} className="px-1 rounded hover:bg-black/40">
                Play
              </button>
              <button type="button" onClick={() => addToQueue(video)} className="px-1 rounded hover:bg-black/40">
                Queue
              </button>
            </div>
          </div>))}
      ...
      {current && (<div className="fixed bottom-3 right-3 w-72 bg-black text-white rounded shadow-lg z-50">
          <YouTubePlayer videoId={current.id}/>
          <div className="flex items-center justify-between px-2 py-1 text-xs">
            <span className="truncate mr-2" title={current.title}>
              {current.title}
            </span>
            <button type="button" onClick={nextVideo} className="bg-gray-700 px-2 py-1 rounded hover:bg-gray-600">
              Next
            </button>
          </div>
        </div>)}
