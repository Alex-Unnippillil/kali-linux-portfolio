...
const PLAYLIST_EMBED_URL =
  'https://open.spotify.com/embed/playlist/37i9dQZF1DXaf6XmhwlgC6?utm_source=generator';
    ...
    <div className={`flex flex-1 flex-col ${className}`}>
      <div className="flex flex-1 min-h-[18rem] overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-[#0b0d11] via-[#07090c] to-[#040506] text-white shadow-[0_32px_64px_rgba(4,6,10,0.7)]">
        ...
        <div className="relative flex-1 bg-black/60">
          <div
            className="pointer-events-none absolute inset-x-0 top-0 h-16 bg-gradient-to-b from-black/60 via-transparent to-transparent"
            aria-hidden
          />
          <iframe
      ...
      <div className="mt-4 flex items-center justify-between rounded-2xl border border-white/5 bg-white/5 px-4 py-3 text-[11px] text-white/70">
        <span>
          Want the full Spotify controls in a separate tab?
        </span>
        <a
          href={PLAYLIST_URL}
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-1 rounded-full bg-[#1DB954] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.28em] text-black transition hover:bg-[#1ed760]"
        >
          Open Spotify
...
export default function SpotifyApp() {
  return (
    <div className="flex h-full w-full flex-col gap-4 bg-[var(--color-bg)] p-5 text-[color:var(--color-text)]">
      <SpotifyWebPlayerLayout description="Open the official Spotify embed inside a dedicated window. Resize, minimise, or pin it anywhere on the desktop while the playlist keeps playing." />
    </div>
  );
}
