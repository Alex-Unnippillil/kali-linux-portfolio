...
const Chrome = () => {
    ...
    const [activeId, setActiveId] = useState(storedActive || tabs[0].id);
    const [address, setAddress] = useState(tabs.find((t) => t.id === activeId)?.url || HOME_URL);
    ...
    const fetchArticle = useCallback(async (tabId, url) => {
        try {
    ...
    const onAddressKey = useCallback((e) => {
        if (e.key === 'Enter')
            navigate(address);
    }, [address, navigate]);
    ...
    const goBack = useCallback(() => {
        setTabs((prev) => prev.map((t) => t.id === activeId && t.historyIndex > 0
            ? { ...t, historyIndex: t.historyIndex - 1, url: t.history[t.historyIndex - 1] }
            : t));
    }, [activeId]);
    const goForward = useCallback(() => {
        setTabs((prev) => prev.map((t) => t.id === activeId && t.historyIndex < t.history.length - 1
            ? { ...t, historyIndex: t.historyIndex + 1, url: t.history[t.historyIndex + 1] }
            : t));
    }, [activeId]);
    useEffect(() => {
    ...
    const toggleMute = useCallback(() => {
        setTabs((prev) => prev.map((t) => (t.id === activeId ? { ...t, muted: !t.muted } : t)));
        try {
            // @ts-ignore
            iframeRef.current.muted = !activeTab.muted;
        }
    ...
    const blockedView = (<div className="flex flex-col items-center justify-center w-full h-full text-center p-4">
      <p className="mb-2">This site refused to connect.</p>
      <a href={activeTab.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">
        Open externally
      </a>
    </div>);
    return (<div className="flex flex-col w-full h-full bg-ub-cool-grey text-white">
      <div className="flex items-center bg-gray-800 text-sm p-1 space-x-1">
        ...
        <input className="flex-grow px-2 py-0.5 text-black rounded" value={address} onChange={(e) => setAddress(e.target.value)} onKeyDown={onAddressKey} spellCheck={false}/>
      ...
      <div className="flex space-x-1 bg-gray-700 text-sm overflow-x-auto">
        {tabs.map((t) => (<div key={t.id} className={`flex items-center px-2 py-1 cursor-pointer ${t.id === activeId ? 'bg-gray-600' : 'bg-gray-700'} `} onClick={() => setActiveId(t.id)}>
      ...
      <div className="flex items-center bg-gray-800 text-sm p-1 space-x-1">
        <input placeholder="Find in page" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && doFind()} className="px-2 py-0.5 text-black rounded flex-grow"/>
      ...
      <div className="flex-grow bg-white relative overflow-auto">
        {articles[activeId] ? (<main style={{ maxInlineSize: '60ch', margin: 'auto' }} dangerouslySetInnerHTML={{ __html: articles[activeId] }}/>) : activeTab.blocked ? (blockedView) : (<iframe ref={iframeRef} src={activeTab.url} title={activeTab.url} className="w-full h-full" sandbox="allow-same-origin allow-scripts allow-forms allow-popups" allow="accelerometer; autoplay; clipboard-write; encrypted-media; geolocation; gyroscope; picture-in-picture; microphone; camera" referrerPolicy="no-referrer" allowFullScreen/>)}
      </div>
    </div>);
};
