  ...
  return (
    <div className="relative inline-flex">
      <button
        ...
        className="flex items-center gap-2 rounded-md border-b-2 border-transparent px-2 py-1 text-sm outline-none transition duration-200 ease-out hover:bg-[color:var(--kali-panel-highlight)] focus-visible:ring-2 focus-visible:ring-[color:var(--kali-blue)] focus-visible:ring-opacity-40"
        tabIndex={isOpen ? -1 : 0}
      >
        <svg
          aria-hidden="true"
      ...
      {isVisible && (
        <div
          ...
          className={`fixed z-[260] flex max-h-[80vh] w-[min(100vw-1.5rem,680px)] flex-col overflow-x-hidden overflow-y-auto rounded-xl border border-[color:var(--kali-panel-border)] bg-[color:var(--kali-panel)] text-white shadow-[0_20px_40px_rgba(0,0,0,0.45)] backdrop-blur-lg transition-all duration-200 ease-out sm:absolute sm:top-full sm:left-0 sm:mt-1 sm:w-[680px] sm:max-h-[440px] sm:flex-row sm:overflow-hidden ${isOpen ? 'opacity-100 translate-y-0 scale-100' : 'pointer-events-none opacity-0 -translate-y-2 scale-95'
            }`}
        ...
        >
          <div className="order-2 flex max-h-[44vh] flex-1 flex-col bg-[color:var(--color-ub-cool-grey)] sm:order-2 sm:max-h-full">
            <div className="border-b border-[color:var(--kali-panel-border)] px-4 py-4 sm:px-5">
              <div className="relative mb-4">
                <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[color:var(--kali-blue)]">
                ...
                <input
                  ref={searchInputRef}
                  className="h-11 w-full rounded-lg border border-transparent bg-[color:var(--color-ub-warm-grey)] pl-10 pr-4 text-sm text-gray-100 shadow-inner transition focus:border-[color:var(--kali-blue)] focus:outline-none focus:ring-2 focus:ring-[color:var(--kali-blue)] focus:ring-opacity-30"
                  type="search"
                  inputMode="search"
                  enterKeyHint="search"
              ...
              <div className="flex flex-wrap items-center gap-3">
                {favoriteApps.slice(0, 6).map((app) => (
                  <button
                    ...
                    className="flex h-11 w-11 items-center justify-center rounded-lg bg-[color:var(--color-ub-warm-grey)] text-white backdrop-blur-sm transition-all duration-200 ease-out hover:-translate-y-0.5 hover:bg-[color:var(--kali-panel-highlight)] hover:shadow-[0_8px_18px_rgba(15,148,210,0.16)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--kali-blue)] focus-visible:ring-opacity-40 focus-visible:ring-offset-2 focus-visible:ring-offset-[color:var(--color-ub-cool-grey)]"
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 sm:px-3">
              {currentApps.length === 0 ? (
                <div className="flex h-full flex-col items-center justify-center gap-3 text-sm text-gray-500">
                  <span className="flex h-12 w-12 items-center justify-center rounded-full bg-[color:var(--color-ub-warm-grey)] text-[color:var(--kali-blue)]">
                    <svg
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                ...
                <ul className="space-y-2" data-testid="whisker-menu-app-list">
                  {currentApps.map((app, idx) => (
                    <li key={app.id}>
                      <button
                        type="button"
                        className={`flex w-full min-h-[44px] items-center justify-between gap-4 rounded-xl px-4 py-3 text-left text-sm backdrop-blur-sm transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--kali-blue)] focus-visible:ring-opacity-40 focus-visible:ring-offset-2 focus-visible:ring-offset-[color:var(--color-ub-cool-grey)] ${idx === highlight
                            ? 'bg-[color:var(--kali-panel-highlight)] text-white shadow-[0_0_0_1px_var(--kali-panel-border)]'
                            : 'text-gray-200 hover:-translate-y-0.5 hover:bg-[color:var(--kali-panel-highlight)] hover:shadow-[0_8px_20px_rgba(15,148,210,0.14)]'
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-[color:var(--color-ub-warm-grey)]">
                            <Image
                              src={app.icon}
                              alt=""
                              width={28}
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <p className="text-xs uppercase tracking-[0.25em] text-[color:var(--kali-blue)]">Application</p>
                          </div>
                        </div>
                        <svg
                          className="h-4 w-4 flex-shrink-0 text-[color:var(--kali-blue)]"
                          viewBox="0 0 24 24"
                          fill="none"
                          stroke="currentColor"
                          strokeWidth="2"
          ...
          <div className="order-1 flex w-full max-h-[36vh] flex-col overflow-y-auto bg-gradient-to-b from-[color:var(--color-ub-warm-grey)] via-[color:var(--color-ub-cool-grey)] to-[color:var(--color-ub-grey)] sm:order-1 sm:max-h-[420px] sm:w-[260px] sm:overflow-visible">
            <div className="flex items-center gap-2 border-b border-[color:var(--kali-panel-border)] px-4 py-3 text-xs uppercase tracking-[0.2em] text-[color:var(--kali-blue)]">
              <span className="inline-flex h-2 w-2 rounded-full bg-[color:var(--kali-blue)]" aria-hidden />
              Categories
            </div>
            <div
              ref={categoryListRef}
              className="-mx-1 flex gap-2 overflow-x-auto px-2 pb-3 pt-3 touch-pan-x sm:mx-0 sm:max-h-full sm:flex-1 sm:flex-col sm:gap-1 sm:overflow-y-auto sm:px-2 sm:py-3"
              role="listbox"
            ...
            >
              {categoryConfigs.map((cat, index) => (
                <button
                  ...
                  type="button"
                  className={`group inline-flex min-h-[48px] min-w-[48px] flex-shrink-0 items-center gap-3 rounded-full border border-transparent bg-[color:var(--color-ub-warm-grey)] px-5 py-2 text-sm text-left backdrop-blur-sm transition-all duration-200 ease-out focus:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--kali-blue)] focus-visible:ring-opacity-40 focus-visible:ring-offset-2 focus-visible:ring-offset-[color:var(--color-ub-cool-grey)] sm:min-h-[44px] sm:min-w-0 sm:w-full sm:rounded-lg sm:px-3 sm:py-3 ${category === cat.id
                      ? 'bg-[color:var(--kali-panel-highlight)] text-white shadow-[0_0_0_1px_var(--kali-panel-border)]'
                      : 'text-gray-300 hover:-translate-y-0.5 hover:bg-[color:var(--kali-panel-highlight)] hover:text-white hover:shadow-[0_8px_18px_rgba(15,148,210,0.16)]'
                    }`}
                ...
                >
                  <span className="hidden w-8 font-mono text-[11px] uppercase tracking-[0.2em] text-[color:var(--kali-blue)] sm:inline-flex">{String(index + 1).padStart(2, '0')}</span>
            ...
            <div className="border-t border-[color:var(--kali-panel-border)] px-4 py-3 text-sm text-gray-400">
              <div className="flex items-center gap-2">
                <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-[color:var(--color-ub-warm-grey)] text-sm font-semibold uppercase text-[color:var(--kali-blue)]">k</span>
                <div>
