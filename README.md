...
| `NEXT_SECURITY_HEADERS` | Optional toggle. Set to `off` to disable CSP/security headers for local debugging. |
...
Defined in `next.config.js` via `lib/csp.js`. See [CSP External Domains](#csp-external-domains) for allowed hostnames:

- **Content-Security-Policy (CSP)** (generated by `createContentSecurityPolicy()`; see [CSP External Domains](#csp-external-domains))
...
> Inline `<script>` tags must call `getCspNonce()` and pass the returned value to a `nonce` attribute. Set `NEXT_SECURITY_HEADERS=off` to disable the headers locally when debugging, but re-enable before committing.
...
| Domain / Pattern | Directive(s) | Purpose |
| --- | --- | --- |
| `platform.twitter.com` | `script-src`, `connect-src`, `frame-src` | Twitter widgets and scripts |
| `syndication.twitter.com` | `script-src`, `connect-src`, `frame-src` | Twitter embed bootstrap |
| `cdn.syndication.twimg.com` | `script-src`, `connect-src` | Twitter CDN assets |
| `*.twitter.com` | `script-src`, `connect-src`, `frame-src` | Additional Twitter content |
| `*.x.com` | `script-src`, `connect-src`, `frame-src` | X (Twitter) domain equivalents |
| `vercel.live` | `script-src`, `frame-src` | Vercel toolbar |
| `*.vercel-scripts.com` | `script-src`, `connect-src` | Vercel Analytics and Speed Insights scripts |
| `*.vercel-insights.com` | `connect-src` | Vercel Analytics ingestion |
| `www.googletagmanager.com` | `script-src` | Google Analytics loader |
| `www.google-analytics.com` | `connect-src` | Google Analytics collection endpoint |
| `www.google.com` | `script-src`, `connect-src` | Google embeds and search widgets |
| `www.googleapis.com` | `connect-src` | YouTube Data API |
| `*.google.com` | `connect-src`, `frame-src` | Chrome app demo origin |
| `www.gstatic.com` | `script-src` | Google assets for embeds |
| `cdn.jsdelivr.net` | `script-src` | Math.js and other libraries |
| `cdnjs.cloudflare.com` | `script-src` | PDF.js worker |
| `stackblitz.com` | `connect-src`, `frame-src` | StackBlitz IDE embeds |
| `example.com` | `connect-src`, `frame-src` | Chrome app demo origin |
| `developer.mozilla.org` | `connect-src`, `frame-src` | Chrome app demo origin |
| `en.wikipedia.org` | `connect-src`, `frame-src` | Chrome app demo origin |
| `ghbtns.com` | `frame-src` | GitHub button embeds |
| `open.spotify.com` | `frame-src` | Spotify embeds |
| `todoist.com` | `frame-src` | Todoist iframe demo |
| `www.youtube.com` | `script-src`, `connect-src`, `frame-src` | YouTube embeds and feeds |
| `www.youtube-nocookie.com` | `script-src`, `frame-src` | Privacy-enhanced YouTube embeds |
| `ipapi.co` | `connect-src` | IP geolocation lookup |
| `api.openweathermap.org` | `connect-src` | Weather API |
| `api.open-meteo.com` | `connect-src` | Weather API |
| `unpkg.com` | `connect-src` | WASM asset downloads |
| `api.github.com` | `connect-src` | GitHub star counts |
| `piped.video` | `connect-src` | Alternative YouTube API |
| `fonts.gstatic.com` | `font-src` | Font assets |

`blob:` URLs are also allowed for `frame-src` and `worker-src` so sandboxed plugins and generated workers can execute safely.
