...
async function maybeKillStaleNextDev({ distDir }) {
  ...
  const netstat = await runCmdCapture("cmd.exe", [
    "/d",
    "/s",
    "/c",
    "netstat -ano -p tcp",
  ...
  if (netstat.code === 0) {
    for (const port of portsToCheck) {
      const re = new RegExp(
        String.raw`:${port}\s+.*\s+LISTENING\s+(\d+)\s*$`,
        "gim",
      );
      let m;
  ...
  for (const pid of pids) {
    ...
    const looksLikeThisRepoNext =
      ...
      (markers.some((m) => haystack.includes(m)) ||
        haystack.includes("kali-linux-portfolio"));
