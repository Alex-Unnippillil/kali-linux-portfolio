...
## Module boundary guard (2025-09-19)

- `yarn test:modules` runs sample ESM scripts via `node --run` and captures the `next build` output to `test-results/module-tests/next-build.log`.
- The test fails if the log contains module-related warnings (e.g., "Module not found" or "Module parse failed").
